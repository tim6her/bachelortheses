...
\begin{figure}
\begin{center}
\subfloat[$\varpikern_3(b)=(1\,3)(2\,4)(5\,6)$\label{g:pi b}]{
...
\subfloat[$\varpikern_3(u)=(1\,2)(3\,4)$\label{g:pi u}]{
...
\subfloat[$\varpikern_3(v)=(1\,2)(5\,7)(6\,8)$\label{g:pi v}]{
...
\subfloat[$\varpikern_3(w)=(5\,6)(7\,8)$\label{g:pi w}]{
...
\begin{align*}
[K\St[\Gamma]{3}:\St[\Gamma]{3}]&=[K:K\cap\St[\Gamma]{3}]=\lvert \pi(K)\rvert=\\
        &=\lvert\varpikern_3(K)\rvert=2^3=8.
\end{align*}
It follows that
\begin{equation*}
[\Gamma:K\St[\Gamma]{3}]=\frac{[\Gamma:\St[\Gamma]{3}]}{[K\St[\Gamma]{3}:\St[\Gamma]{3}]}=\frac{2^7}{2^3}=2^4,
\end{equation*}
while \ref{p:Index of K in Gamma} implies $[\Gamma:K]=2^4.$ Thus, the obvious inclusion $K\subseteq \St[\Gamma]{3}$ is in fact an equality.
...
\psi_1(aca(wcw^{-1}c)aca)	&=\psi(aca)\psi_1(wcw^{-1}c)\psi_1(aca)=\\
							...
							&=(\id,u)\in\psi_1(K).
\end{align*}
As a consequence of \cref{thm:Conjugation with a inverts the performing of b c d}, the images of the automorphisms $awa,awcw^{-1}ca$ and $ca(wcw^{-1}c)ac\in K$ generate $K\times\id$. Thus, $K\times K\subseteq\psi_1(K)$.
...
\item We apply the formula obtained in \cref{thm:psi k} to compute the image $\psi_3((ab)^8)$. Since the number of occurrences of automorphism $a$ in $(ab)^8$ is even, $(ab)^8\in\St[\Gamma]{1}$ by \cref{thm:StGamma1 is a proper subset of Gamma}. We find
\begin{align*}
...
\begin{align*}
\psi_3	&((ab)^8)=\\
            &=(\psi_1(adad),\psi_1(dada),\psi_1(dada),\psi_1(adad))_3=\\
            &=((b,\id)(\id,b),(\id,b)(b,\id),\\
            &\quad \quad \quad (\id,b)(b,\id),(b,\id)(\id,b))_3=\\
			&=(b,b,b,b,b,b,b,b).
\end{align*}
