...
\item Let $G, H$ be two $\Z$-modules like the one defined in Example~\secref{ex.module}{ex.Z module}, then every group-homomorphism $\varphi\colon G\rightarrow H$ is a module-homomorphism. To prove this one verifies $\varphi(rx)=r\varphi(x)$ for all $r\in\Z$, $z\geq 0$ per induction on $r$. If $r<0$, we note that 
\begin{equation*}
...
\begin{lemma}\label{lm.Composition of Homo} Let $M, N, O$ be $R$-modules and $\varphi\colon M\rightarrow N$, $\psi\colon N\rightarrow O$ homomorphisms, then $\psi\circ\varphi\colon M\rightarrow O$ is a homomorphism.
\end{lemma}
\begin{proof}
...
\begin{prop}\label{prop.Homomorphisms are Module}
...
\begin{rem}
Let $M$ a module over a commutative unital ring $R$. If we view $R$ as a module over itself like we did in Example~\secref{ex.module}{ex.RingAsModule}, Proposition~\ref{prop.Homomorphisms are Module} implies that $\Hom_R(R,M)$ is an $R$-module. The mapping $F\colon\Hom_R(R,M)\to M$ defined by $\varphi\mapsto\varphi(1)$ is a homomorphism, because 
\begin{equation*}
